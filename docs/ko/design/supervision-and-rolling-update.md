...
이미 `plana` crate을 프로토콜 계층으로 공유하고 있습니다. 바로 이 균질성이
...
## 4. crate 소유권
"plana에 넣어라"라는 사용자 목표는 분할되어야 합니다. 왜냐하면 **현재 plana는
...
분할:
- **`plana::lifecycle`(프로토콜 계약, plana에 배치).** JSON-RPC 메서드와
  타입만: `DrainState`, `ReadyStatus`, `Lifecycle.Drain`, `Lifecycle.Status`,
  `Worker.Status` 등. plana의 "양쪽에 쌍으로 존재" 규칙을 만족합니다.
- **`malkuth`(신규 crate, 런타임).** `plana` 프로토콜 타입 + `tokio`
...
(§8 매트릭스 참조). 모든 것을 plana에 넣으면 그것을 "프로토콜 + 선택적
...
1. **단계 A — 세 프로젝트에 걸친 Layer 1.** 시그널 시맨틱 + `/healthz` /
   ...
   먼저 수정).
2. **단계 B — `plana::lifecycle` 프로토콜 + `malkuth` 골격.** trait
