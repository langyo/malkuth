...
であり、プロトコル層としてすでに `plana` crate を共有している。この均質性
...
## 4. crate の帰属
ユーザ目標である「plana に入れる」は分割しなければならない。なぜなら、**plana
...
分割:
- **`plana::lifecycle` (プロトコル契約、plana に置く)。** JSON-RPC のメソッドと
  型のみ: `DrainState`、`ReadyStatus`、`Lifecycle.Drain`、`Lifecycle.Status`、
  `Worker.Status` など。plana の「両サイドでペアになる」というルールを満たす。
- **`malkuth` (新規 crate、ランタイム)。** `plana` プロトコル型 +
...
(§8 のマトリクスを参照)。すべてを plana に押し込めば、plana を「プロトコル +
...
2. **フェーズ B —— `plana::lifecycle` プロトコル + `malkuth`
