...
`plana` crate 作為協議層。正是這種同構,使得*做一套*監督機制、三處複用
是值得的。
...
正是"原理相通"的落點。
## 4. crate 歸屬
使用者目標"放進 plana"必須拆分,因為 **plana 當前是純協議/型別 crate**——
...
處理)會破壞它輕量、可釋出的定位。
拆分:
- **`plana::lifecycle`(協議契約,放進 plana)。** 只放 JSON-RPC 方法與型別:
  `DrainState`、`ReadyStatus`、`Lifecycle.Drain`、`Lifecycle.Status`、
  `Worker.Status` 等。符合 plana"雙方配對"的規則。
- **`malkuth`(新 crate,執行時)。** 依賴 `plana` 協議型別 +
...
plana 會迫使它變成"協議 + 可選執行時",毀掉其純淨性——不推薦。
## 5. 核心抽象
...
2. **階段 B —— `plana::lifecycle` 協議 + `malkuth` 骨架。** trait
